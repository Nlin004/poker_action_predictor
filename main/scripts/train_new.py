...
class MLPBaseline(nn.Module):
    def __init__(self, num_input=3, hist_vocab=7, hist_emb_dim=8, hist_len=10, 
                 hidden=64, num_classes=len(ACTION_SET)):
        super().__init__()
...
class TransformerActionModel(nn.Module):
    """
    Self-contained Transformer-based per-decision action predictor.

    Inputs:
      - num: [B, num_input] (numeric features, e.g. 13)
      - hist: [B, L] (history token ids, padded with pad_token_id)
    Output:
      - logits: [B, num_classes]
    Design choices:
      - grouped numeric encoding (position / betting / game state)
      - token embedding -> transformer encoder -> CLS + mean pooling
      - project transformer outputs to the numeric embedding size and run cross-attention
      - simpler classifier stack (no external helper classes)
    """
    def __init__(
        self,
        num_input=13,
        ...
        dim_feedforward=512,
        dropout=0.1,
        cross_attn_dim=128,   # embedding dim used for cross-attention / numeric projection
        num_classes=6,
        pad_token_id=6
    ):
        super().__init__()
        ...
        # ---- history token processing ----
        self.token_emb = nn.Embedding(hist_vocab, hist_emb_dim, padding_idx=pad_token_id)
        self.token_proj = nn.Linear(hist_emb_dim, d_model)   # bring token emb to d_model
        # learnable CLS token
        self.cls_token = nn.Parameter(torch.randn(1, 1, d_model))
        # sinusoidal positional encoding buffer
        self.register_buffer("pos_encoding", self._build_sinusoidal(hist_len + 1, d_model))
        ...
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=num_encoder_layers, norm=nn.LayerNorm(d_model))

        # final norm on transformer outputs
        self.final_norm = nn.LayerNorm(d_model)

        # ---- numeric grouped processing (Option A) ----
        # position features: position, is_button, is_sb, is_bb  -> 4 -> 32
        self.position_proj = nn.Sequential(
            nn.Linear(4, 32), nn.LayerNorm(32), nn.GELU()
        )
        # betting features: pot_norm, stack_to_bb, to_call_norm, pot_odds, spr, raises_this_street -> 6 -> 64
        self.betting_proj = nn.Sequential(
            nn.Linear(6, 64), nn.LayerNorm(64), nn.GELU()
        )
        # game-state features: num_active, street, community_cards -> 3 -> 32
        self.game_state_proj = nn.Sequential(
            nn.Linear(3, 32), nn.LayerNorm(32), nn.GELU()
        )
        # combine numeric groups -> compact vector
        self.num_proj_head = nn.Sequential(
            nn.Linear(32 + 64 + 32, cross_attn_dim),
            nn.LayerNorm(cross_attn_dim),
            nn.GELU(),
            nn.Dropout(dropout),
        )  # produces (B, cross_attn_dim)

        # ---- cross-attention ----
        # numeric features (query) attend to history (key/value).
        # MultiheadAttention requires query/key/value same embed dim -> use cross_attn_dim.
        self.history_to_cross = nn.Linear(d_model, cross_attn_dim)  # project transformer outputs -> cross_attn_dim
        self.cross_attn = nn.MultiheadAttention(embed_dim=cross_attn_dim, num_heads=4, dropout=dropout, batch_first=True)
        self.cross_norm = nn.LayerNorm(cross_attn_dim)

        # ---- classifier head ----
        combined_dim = d_model + cross_attn_dim
        self.classifier = nn.Sequential(
            nn.Linear(combined_dim, dim_feedforward // 2),
    ...
    def _build_sinusoidal(self, max_len, d_model):
        # returns tensor shape (1, max_len, d_model)
    ...
    def forward(self, num, hist):
        """
        num: [B, num_input]  (expects >=13 when using grouped indices)
        hist: [B, L]         (token ids)
        """
        B, L = hist.shape

        # --- token embedding + projector + positional + CLS ---
        t = self.token_emb(hist)                # [B, L, hist_emb_dim]
        t = self.token_proj(t)                  # [B, L, d_model]
        cls = self.cls_token.expand(B, -1, -1)  # [B, 1, d_model]
        x = torch.cat([cls, t], dim=1)          # [B, L+1, d_model]

        # add positional encoding (ensure pos buffer length covers L+1)
        pos = self.pos_encoding[:, : L + 1, :].to(x.device)
        x = x + pos
        # transformer padding mask: True where tokens are padding (including hist pad; CLS is valid)
        key_padding_mask = torch.cat([
            torch.zeros(B, 1, dtype=torch.bool, device=hist.device),
            (hist == self.pad_token_id)
        ], dim=1)  # [B, L+1]

        # transformer encoder
        x = self.transformer(x, src_key_padding_mask=key_padding_mask)   # [B, L+1, d_model]
        x = self.final_norm(x)

        # pooling: CLS token + mean over non-masked positions (exclude CLS)
        cls_out = x[:, 0, :]                                # [B, d_model]
        if key_padding_mask[:, 1:].all():
            # entire history padded -> mean is zeros
            mean_pool = torch.zeros_like(cls_out)
        else:
            mask_inv = (~key_padding_mask[:, 1:]).float().unsqueeze(-1)  # [B, L, 1]
            enc_hist = x[:, 1:, :]                                        # [B, L, d_model]
            mean_pool = (enc_hist * mask_inv).sum(dim=1) / mask_inv.sum(dim=1).clamp(min=1.0)  # [B, d_model]

        pooled_history = cls_out + mean_pool   # simple combination -> [B, d_model]

        # --- numeric grouped processing (Option A) ---
        # indexing must match your numeric layout in dataset (this matches your grouping)
        # position: cols 6:10; betting: [0,1,3,4,5,12]; game_state: [2,10,11]
        pos_feats = num[:, 6:10]                         # [B, 4]
        bet_feats = num[:, [0, 1, 3, 4, 5, 12]]          # [B, 6]
        state_feats = num[:, [2, 10, 11]]                # [B, 3]

        pos_emb = self.position_proj(pos_feats)         # [B, 32]
        bet_emb = self.betting_proj(bet_feats)          # [B, 64]
        state_emb = self.game_state_proj(state_feats)   # [B, 32]

        num_combined = torch.cat([pos_emb, bet_emb, state_emb], dim=1)  # [B, 128?]
        num_proj = self.num_proj_head(num_combined)                     # [B, cross_attn_dim]

        # --- cross-attention: numeric (query) <-> history (key/value) ---
        # Project history tokens (exclude CLS) to cross_attn_dim
        hist_kv = self.history_to_cross(x[:, 1:, :])   # [B, L, cross_attn_dim]
        # run multihead attention. Query shape: [B, 1, cross_attn_dim], key/value: [B, L, cross_attn_dim]
        q = num_proj.unsqueeze(1)                      # [B, 1, cross_attn_dim]
        # key_padding_mask for history (exclude CLS): [B, L]
        hist_key_pad = key_padding_mask[:, 1:]         # True for padding tokens
        attn_out, _ = self.cross_attn(query=q, key=hist_kv, value=hist_kv, key_padding_mask=hist_key_pad)
        attn_out = attn_out.squeeze(1)                 # [B, cross_attn_dim]
        num_crossed = self.cross_norm(num_proj + attn_out)   # residual + norm

        # --- combine pooled history and numeric rep ---
        combined = torch.cat([pooled_history, num_crossed], dim=1)  # [B, d_model + cross_attn_dim]
...
class FocalLoss(nn.Module):
    ...
    Better than standard CrossEntropy for poker where some actions are rare.
    """
...
class MixupTransform:
    ...
    Mixes two examples together to create synthetic training data.
    """
    def __init__(self, alpha=0.2):
        self.alpha = alpha
    
    def __call__(self, x1, x2, y1, y2):
        """Mix two batches together."""
...
def main():    
    ...
    p.add_argument("--mixup", action="store_true", help="enable mixup")
    args = p.parse_args()
    ...
    # Split by game_id
    # game_ids = df["game_id"].unique()
    # train_gids, val_gids = train_test_split(game_ids, test_size=0.2, random_state=42)

    # train_df = df[df["game_id"].isin(train_gids)].reset_index(drop=True)
    # val_df = df[df["game_id"].isin(val_gids)].reset_index(drop=True)
    # print(f"Train: {len(train_df)}, Val: {len(val_df)}")
    
    # Create datasets (use enhanced version!)
    if args.model in ["mlp", "transformer"]:
        train_ds = NextActionDataset(train_df, grouped=False)
        val_ds = NextActionDataset(val_df, grouped=False)
        collate = collate_fn_mlp
    else:
        train_ds = NextActionDataset(train_df, grouped=True)
        val_ds = NextActionDataset(val_df, grouped=True)
        collate = collate_fn_seq
    ...
    classes = train_df["action_label"].astype(int).values
    num_classes = len(ACTION_SET)   # ALWAYS 6

    # always build 6-length bincount even if dataset only has {0,1,2}
    ...
    # classes = train_df["action_label"].values
    ...
    if args.model == "transformer":
        model = TransformerActionModel(
            ...
            dim_feedforward=512,
            dropout=0.2,
            stochastic_depth=0.1,  # NEW
            num_classes=num_classes
        )
        # model = TransformerActionModel(
        #     num_input=13,  # INCREASED! I used to have this at 3, for just [pot_norm, stack_to_bb, num_active] but I feel like more is better for transformers
        #     hist_vocab=7,
        #     hist_emb_dim=64,
        #     hist_len=10,
        #     d_model=256,
        ...
        model = ContextualLSTM(num_input=13)
    else:
        # Use existing MLP but with 13 features
        model = MLPBaseline(num_input=13, hidden=128)
    
    model.to(device)
    crit = FocalLoss(alpha=class_weights, gamma=2.0)

    # Use training with mixup
    
    # Optimizer with weight decay
    opt = create_improved_optimizer(model, lr=args.lr, weight_decay=args.weight_decay)
    
    # Loss with label smoothing
    # crit = LabelSmoothingCrossEntropy(smoothing=args.label_smoothing)
    ...
    #     #     scheduler=scheduler, gradient_clip=args.gradient_clip
    #     # )
    ...
    # ax1.plot(metrics_df["epoch"], metrics_df["train_acc"], label="Train")
    # ax1.plot(metrics_df["epoch"], metrics_df["val_acc"], label="Val")
    ...
    # ax2.plot(metrics_df["epoch"], metrics_df["train_loss"], label="Train")
    # ax2.plot(metrics_df["epoch"], metrics_df["val_loss"], label="Val")
...
def new_main():
    ...
    if args.model == "transformer":
        ...
        model = ContextualLSTM(num_input=13, num_classes=num_classes)
    else:
        model = MLPBaseline(num_input=13, hidden=128)
